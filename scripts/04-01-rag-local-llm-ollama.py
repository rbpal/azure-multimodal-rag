...
Answer:"""
    ...
    def simple_diagnostic(self, question: str):
        """Show the complete prompt sent to model and the model's response"""
...
Answer:"""
        ...
        complete_prompt = prompt_template.format(context=context, question=question)
    ...
    def interactive_chat(self):
        ...
        print("Special: 'debug <question>' for prompt/response analysis")
        print("="*90 + "\n")
        ...
        while True:
            try:
                ...
                if question.lower().startswith('debug '):
                    debug_question = question[6:].strip()  # Remove 'debug ' prefix
                    if debug_question:
                        self.simple_diagnostic(debug_question)
                    else:
                        print("❌ Please provide a question after 'debug'. Example: debug What is Azure Load Balancer?")
                    continue
    ...
    try:
        ...
        while True:
            ...
            if choice == '1':
                ...
                question = input("Enter question for prompt/response analysis: ").strip()
                if question:
                    rag_system.simple_diagnostic(question)
                else:
